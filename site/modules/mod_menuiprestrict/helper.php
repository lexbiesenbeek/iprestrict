            ...
            if ($items) {
                foreach ($items as $i => $item) {
                    ...
                    if ($app->getUserState('iprestriction', 0) == 1) {
                        // check if the current menuitem points to an article
                        // @todo are there more ways to point to a single article? e.g. "archived", "special"?
                        $arrayCheck = ['option' => 'com_content', 'view' => 'article'];
                        if (
                            $item->type == 'component' &&
                            array_intersect_assoc($item->query, $arrayCheck) === $arrayCheck
                        ) {
                            // load article
                            JModelLegacy::addIncludePath(JPATH_SITE . '/components/com_content/models', 'bla');
                            $model = JModelLegacy::getInstance('Article', 'contentModel');
                            $article = $model->getItem($item->query['id']);
                            // check if it is ip restricted
                            if (strpos($article->attribs, '"iprestrict":"1"')) {
                                // unset the article menu-item
                                unset($items[$i]);
                                continue;
                            }
                        }
